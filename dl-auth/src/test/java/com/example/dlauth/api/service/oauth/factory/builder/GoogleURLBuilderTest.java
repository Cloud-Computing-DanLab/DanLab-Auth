package com.example.dlauth.api.service.oauth.factory.builder;
...
class GoogleURLBuilderTest extends IntegrationHelper {
    @Autowired
    private GoogleURLBuilder urlBuilder;

    @Value("${oauth2.client.google.client-id}") private String clientId;
    @Value("${oauth2.client.google.client-secret}") private String clientSecret;
    @Value("${oauth2.client.google.redirect-uri}") private String redirectUri;

    @Value("${oauth2.provider.google.authorization-uri}") String authorizationUri;
    @Value("${oauth2.provider.google.token-uri}") private String tokenUri;
    @Value("${oauth2.provider.google.profile-uri}") private String profileUri;
    ...
    void authorizeURIBuildSuccess() {
        ...
        assertThat(authorizeURL).isEqualTo(authorizationUri
                ...
                + "&state=" + state
                + "&scope=email+profile");  // openid가아닌 email+profile 변경
    }
